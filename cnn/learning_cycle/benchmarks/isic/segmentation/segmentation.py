...
from segmentation.future import graph
from segmentation.utils import compare_jaccard
...
def entropy_rag(graph, labels, image,
    ...
    for x, y, d in graph.edges_iter(data=True):
        d['weight'] = mutual_information(graph.nodes[x]['pixels'],
                                         graph.nodes[y]['pixels'],
                                         graph.nodes[x]['entropy'],
                                         graph.nodes[y]['entropy'])


def _weight_entropy(graph, src, dst, n):
    return mutual_information(graph.nodes[dst]['pixels'],
                              graph.nodes[n]['pixels'],
                              graph.nodes[dst]['entropy'],
                              graph.nodes[n]['entropy'])
...
def merge_entropy(graph, src, dst, image, labels):
    graph.nodes[dst]['pixels'] = np.hstack((graph.nodes[dst]['pixels'], graph.nodes[src]['pixels']))
    if graph.nodes[dst]['pixels'].size == 0:
        graph.nodes[dst]['entropy'] = 0
    else:
        hist, _ = np.histogram(graph.nodes[dst]['pixels'], bins=np.arange(-0.5, 256))
        graph.nodes[dst]['entropy'] = entropy(hist)

def _weight_haralick(graph, src, dst, n):
    nodedst = graph.nodes[dst]['haralick']
    noden = graph.nodes[n]['haralick']
