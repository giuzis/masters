...
do
    for val in ${optArray[@]}
    do
        python3 isic.py with _model_name="$model" _optimizer="$val" _batch_size=4 _lr_init=0.001
    ...
    do
        ...
        python3 isic.py with _model_name="$model" _optimizer="$val" _batch_size=32 _lr_init=0.001
    done

    for val in ${optArray[@]}
    do
        # python isic.py with _model_name=resnext101_32x4d _optimizer="$val" _batch_size=4 _lr_init=0.001
        python3 isic.py with _model_name="$model" _optimizer="$val" _batch_size=32 _lr_init=0.0001
    done
done